...
export function _setPermission(obj: MutablePermissions, cid: string, accepts: boolean | string | number | undefined) {
    ...
    if (typeof accepts === 'number') {
        if (accepts === -1) delete obj.permissions[cid];
        else obj.permissions[cid] = accepts;
    }
