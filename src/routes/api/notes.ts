...
notes.post("/:nid", checkAdmin, async (req, res) => {
    ...
    if (note) {
        ...
        note = makeNote((await getNotes(req.body.cid)).length, req.params.nid, req.body.cid, req.body.name, req.body.desc);
...
notes.post("/:nid/upload", checkAdmin, async (req, res) => {
    ...
    if (newNoteSource && "data" in newNoteSource) {
        ...
        // Markdown renderer
        if (newNoteSource.name.toLowerCase().endsWith(".md")) str = marked(str);
